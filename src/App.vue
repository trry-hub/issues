...
const textTracks = [subtitlesVTT(res.transcription.paragraphs), chaptersVTT(res.autoChapters)]
...
<template>
  <media-player class="player" title="Sprite Fight" :src="res.videoInfo.playUrl" crossOrigin playsInline @provider-change="onProviderChange" @can-play="onCanPlay" ref="$player">
    <media-provider>
      <media-poster class="poster" :src="res.videoInfo.coverUrl" alt="Girl walks into campfire with gnomes surrounding her friend ready for their next meal!" />
    </media-provider>

    <VideoLayout :chapters="res.autoChapters" />
  </media-player>
</template>
